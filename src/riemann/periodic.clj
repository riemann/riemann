...
(defn deferrable-every
  "Calls f every interval seconds, until stopped. Returns a control function.
  Can be deferred. I'll probably change this API."
  ...
  ([interval delay f]
   (let [waterline (ref (+ delay (unix-time)))
     ...
     (fn [action & args]
       (case action
         :stop (dosync (ref-set waterline nil))
         :defer (let [seconds (first args)]
