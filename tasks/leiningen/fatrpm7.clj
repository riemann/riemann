  ...
  (map (comp mapping
       ...
       [; Jar
        ...
        {:directory "/usr/lib/systemd/system"
         ...
         :username "root"
         :groupname "root"
         :sources [(source (file (:root project) "pkg" "rpm" "riemann.service")
                           "riemann.service")]}]))
...
(defn extract-rpm
  ...
  (let [dir (file (workarea project)
    ...
    (doseq [rpm rpms]
      (let [rname (replace (.getName rpm) #"noarch" "noarch-EL7")
